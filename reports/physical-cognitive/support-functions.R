...
select_for_table <- function(
  ...
  if(!track=="all"){
    ...
    if(track=="pulmonary"){
      d2 <- d1 %>% dplyr::filter(process_a %in% c("fev", "fev100", "pef", "pek"))
    }
  }else{
    d2 <- d1
  }
...
get_forest_data <- function(
  ...
  if(track=="all"){
    ...
    if(track=="pulmonary"){
      ...
      d1 <- catalog_pretty %>% dplyr::filter(process_a %in% c("fev","fev100", "pef", "pek"))
    }
  }
...
print_forest_plot <- function(
  ...
  d <- data_forest %>%
    dplyr::filter(
       process_b_domain   == domain_
      ,subgroup == subgroup_
    ) %>%
    dplyr::select(process_b_domain,study, 
                  process_a, process_b,
                  # physical, cognitive,
